...
canvasSketch(async (props) => {
  ...
  const solutionLength = siren.packParams().length;
  ...
  window.addEventListener("keydown", (ev) => {
    if (ev.key == "g") {
      ...
      ev.preventDefault();
      const glsl = generateShadertoyGLSL(siren);
      window.glsl = glsl;
  ...
  const forward = (ux, uy, param = 0, out = tmpOutput) => {
    ...
    // const pt = kaleido(ux, uy, 3);
    return siren.forward(pt, out);
  };
  ...
  function generateShadertoyGLSL(siren) {
    const { dims, w0 } = siren;
    const params = siren.packParams();
    let offset = 0;
